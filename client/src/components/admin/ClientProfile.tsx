    ...
    <div>
      ...
      {isEditingProfile ? (
        <input
          ...
          className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-text-primary focus:outline-none focus:border-accent/50 text-sm transition-colors"
        />
      ) : (
        <p className="text-sm text-text-primary py-2 border-b border-white/[0.05]">
          {(client as unknown as Record<string, string | undefined>)[key] || <span className="text-text-muted">—</span>}
        </p>
      )}
    </div>
  )
      ...
      {isEditingScope ? (
        textarea ? (
          <textarea
            ...
            rows={3}
            className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-text-primary focus:outline-none focus:border-accent/50 text-sm transition-colors resize-none"
          />
        ) : (
          <input
            type="text"
        ...
        <p className="text-sm text-text-primary py-2 border-b border-white/[0.05] whitespace-pre-wrap">
          {(client?.projectScope as unknown as Record<string, string | undefined>)?.[key] || <span className="text-text-muted">—</span>}
        </p>
      )}
    </div>
  )
    ...
    >
      ...
      {confirmDelete && (
        <div className="flex items-center gap-3 p-4 bg-red-400/5 border border-red-400/20 rounded-xl">
          ...
          <div className="flex gap-2">
            <button onClick={() => setConfirmDelete(false)} className="text-sm text-text-muted hover:text-text-primary px-3 py-1.5 transition-colors">Cancel</button>
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
          <div className="backdrop-blur-xl bg-white/[0.02] border border-white/[0.08] rounded-xl p-6">
            <div className="flex items-center justify-between mb-6">
              <h3 className="font-semibold text-text-primary flex items-center gap-2">
                <User className="w-4 h-4 text-accent" /> Contact Information
              </h3>
              {isEditingProfile ? (
                <div className="flex gap-2">
                  <button onClick={() => setIsEditingProfile(false)} className="text-sm text-text-muted hover:text-text-primary px-3 py-1.5 transition-colors">Cancel</button>
                  <button onClick={handleSaveProfile} disabled={savingProfile} className="flex items-center gap-1.5 text-sm bg-accent text-black px-4 py-1.5 rounded-lg font-medium hover:bg-accent/90 disabled:opacity-50 transition-colors">
                    <Save className="w-3.5 h-3.5" />
                    {savingProfile ? 'Saving...' : 'Save'}
                  </button>
                </div>
              ) : (
                <button onClick={() => setIsEditingProfile(true)} className="flex items-center gap-1.5 text-sm text-text-muted hover:text-accent transition-colors">
          ...
          <div className="backdrop-blur-xl bg-white/[0.02] border border-white/[0.08] rounded-xl p-6">
            <h3 className="font-semibold text-text-primary flex items-center gap-2 mb-6">
              <Globe className="w-4 h-4 text-accent-secondary" /> Social Media
            </h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4">
              <div>
                <label className="text-xs text-text-muted mb-1 flex items-center gap-1.5"><Instagram className="w-3 h-3" />Instagram</label>
                {isEditingProfile ? (
                  <input value={profileForm.instagram ?? ''} onChange={e => setProfileForm(p => ({ ...p, instagram: e.target.value }))} className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-text-primary focus:outline-none focus:border-accent/50 text-sm" />
                ) : (
                  <p className="text-sm text-text-primary py-2 border-b border-white/[0.05]">{client.instagram || <span className="text-text-muted">—</span>}</p>
                )}
              </div>
              <div>
                <label className="text-xs text-text-muted mb-1 flex items-center gap-1.5"><Twitter className="w-3 h-3" />Twitter / X</label>
                {isEditingProfile ? (
                  <input value={profileForm.twitter ?? ''} onChange={e => setProfileForm(p => ({ ...p, twitter: e.target.value }))} className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-text-primary focus:outline-none focus:border-accent/50 text-sm" />
                ) : (
                  <p className="text-sm text-text-primary py-2 border-b border-white/[0.05]">{client.twitter || <span className="text-text-muted">—</span>}</p>
                )}
              </div>
              <div>
                <label className="text-xs text-text-muted mb-1 flex items-center gap-1.5"><Linkedin className="w-3 h-3" />LinkedIn</label>
                {isEditingProfile ? (
                  <input value={profileForm.linkedin ?? ''} onChange={e => setProfileForm(p => ({ ...p, linkedin: e.target.value }))} className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-text-primary focus:outline-none focus:border-accent/50 text-sm" />
                ) : (
                  <p className="text-sm text-text-primary py-2 border-b border-white/[0.05]">{client.linkedin || <span className="text-text-muted">—</span>}</p>
                )}
              </div>
              <div>
                <label className="text-xs text-text-muted mb-1 flex items-center gap-1.5"><Facebook className="w-3 h-3" />Facebook</label>
                {isEditingProfile ? (
                  <input value={profileForm.facebook ?? ''} onChange={e => setProfileForm(p => ({ ...p, facebook: e.target.value }))} className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-text-primary focus:outline-none focus:border-accent/50 text-sm" />
                ) : (
                  <p className="text-sm text-text-primary py-2 border-b border-white/[0.05]">{client.facebook || <span className="text-text-muted">—</span>}</p>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="backdrop-blur-xl bg-white/[0.02] border border-white/[0.08] rounded-xl p-6">
            ...
            {isEditingProfile ? (
              ...
              <p className="text-sm text-text-primary whitespace-pre-wrap">
                {client.notes || <span className="text-text-muted">No notes yet.</span>}
              </p>
            )}
          </div>
        </motion.div>
      ...
      {activeSection === 'scope' && (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
          <div className="backdrop-blur-xl bg-white/[0.02] border border-white/[0.08] rounded-xl p-6 space-y-6">
            <div className="flex items-center justify-between">
              <h3 className="font-semibold text-text-primary flex items-center gap-2">
                <Briefcase className="w-4 h-4 text-accent" /> Project Scope
              </h3>
              {isEditingScope ? (
                <div className="flex gap-2">
                  <button onClick={() => setIsEditingScope(false)} className="text-sm text-text-muted hover:text-text-primary px-3 py-1.5 transition-colors">Cancel</button>
                  <button onClick={handleSaveScope} disabled={savingScope} className="flex items-center gap-1.5 text-sm bg-accent text-black px-4 py-1.5 rounded-lg font-medium hover:bg-accent/90 disabled:opacity-50 transition-colors">
                    <Save className="w-3.5 h-3.5" />
                    {savingScope ? 'Saving...' : 'Save'}
                  </button>
                </div>
              ) : (
                <button onClick={() => { setIsEditingScope(true); setScopeForm(client.projectScope ?? {}) }} className="flex items-center gap-1.5 text-sm text-text-muted hover:text-accent transition-colors">
                  <Edit3 className="w-3.5 h-3.5" /> Edit
                </button>
              )}
            </div>
            ...
            {/* Status */}
            <div>
              <label className="block text-xs text-text-muted mb-1">Project Status</label>
              {isEditingScope ? (
                <select
                  value={scopeForm.status ?? 'active'}
                  onChange={e => setScopeForm(s => ({ ...s, status: e.target.value }))}
                  className="bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-text-primary focus:outline-none text-sm"
                >
                ...
                <span className={`inline-flex text-xs px-3 py-1 rounded-full font-medium ${
                  client.projectScope?.status === 'active' ? 'bg-green-400/10 text-green-400' :
                  client.projectScope?.status === 'completed' ? 'bg-accent/10 text-accent' :
          ...
          <div className="backdrop-blur-xl bg-white/[0.02] border border-white/[0.08] rounded-xl p-6">
            <h3 className="font-semibold text-text-primary mb-5 flex items-center gap-2">
              <span className="w-4 h-4 text-accent">📁</span> Project Documents
            </h3>
      ...
      {activeSection === 'kanban' && (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
          <div className="mb-4 flex items-center justify-between">
            <h3 className="font-semibold text-text-primary flex items-center gap-2">
              <KanbanSquare className="w-4 h-4 text-accent" /> Task Board
            </h3>
            <p className="text-xs text-text-muted">{client.tasks.length} task{client.tasks.length !== 1 ? 's' : ''} total</p>
          </div>
          <KanbanBoard
            clientId={client.id}
            tasks={client.tasks}
